...
def writer(queue,lock):
    ...
    Introduction
    ------------
    writer, it's a decorative class that produce one fasta sequence from the fasta file
...
#-------------------------------------------------------------------------------------------------------------------
def read_and_print(queue,lock):
    '''
    Introduction
    ------------
    read_and_print, multi consumer may run at the same time 
...
if __name__ == '__main__':
    ...
    process1 = multiprocessing.Process(target=writer,args=(queue,lock))  #single producer
    process1.start()
    for i in range(consumer_num):  #multi consumer
        process2 = multiprocessing.Process(target=read_and_print,args=(queue,lock))
        process2.start()
